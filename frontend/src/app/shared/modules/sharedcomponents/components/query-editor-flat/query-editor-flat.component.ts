...
@Component({
  selector: 'query-editor-flat',
  templateUrl: './query-editor-flat.component.html',
  styleUrls: ['./query-editor-flat.component.scss']
})
export class QueryEditorFlatComponent implements OnInit, OnChanges, OnDestroy {
  @HostBinding('class.query-editor') private _hostClass = true;
  @Input() type;
  @Input() query: any = { metrics: [], namespace: '', filters: [], settings: {visual: {visible: true}}};
  @Input() label = '';
  @Input() edit = [];
  // @Input() edit = [];
  editNamespace = false;
  ...
  // save namespace tp metric
  saveNamespace(namespace: string, index: number ) {
