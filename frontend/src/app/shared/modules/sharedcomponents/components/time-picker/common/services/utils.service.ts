...
export class UtilsService {
  ...
  isTimeStampValid(time: string): boolean{
    return (time && !time.startsWith("-") && Number(time) < maxUnixTimestamp && Number(time) >= minUnixTimestamp && moment.unix(Number(time)).isValid());
  }
  ...
  dateWithoutTimeToMoment(time: string): Moment {
    for (let format in validDateWithoutTimeFormat){
  ...
  strippedRelativeTime(relativeTime: string): string {
    var timeAmount: number = this.getTimeAmount(relativeTime);
    var timeUnit: string = this.getTimeUnitAbbr(relativeTime.split(timeAmount.toString()).pop());
