    ...
    ngOnChanges(changes: SimpleChanges) {
        ...
        const underlayCallback = (canvas, area, g) => {
            if (this.eventBuckets && this.showEvents) {
                ...
                for (const bucket of this.eventBuckets) {
                    const coords = g.toDomCoords(bucket.startTime + bucket.width, 0);
        ...
        if (!changes) {
            ...
            if (!this._g && this.data) {
                // let print the data out here
                console.log('hill - data for dygraph', this.data);
                if (this.data.ts.length === 0) {
                    this.displayNoData();
                } else  {
            ...
            if (this._g && changes.data && changes.data.currentValue) {
                ...
                console.log('hill - new data for dygraph', ndata, ndata.ts.length);
                if (ndata.ts.length === 0) {
                    this.displayNoData();
                } else {
